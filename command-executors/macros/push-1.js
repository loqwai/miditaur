...
const macros = {
  ...
  65:{
    name: "Helldivers: Eagle Strafing Run",
    keys: [Key.W, Key.D, Key.D],
  },
  96:
  {
    name: "Helldivers: Reinforce",
    keys: [Key.W,Key.S, Key.D, Key.A, Key.W],
  },
  97: {
    "name": "Helldivers: Resupply",
    "keys": [Key.S, Key.S, Key.W, Key.D]
  },
  60: {
    name: "Helldivers: Machine Gun",
    keys: [Key.S, Key.A, Key.S, Key.W, Key.D],
  ...
  async setAllColors() {
    ...
    for (let x = 0; x < 8; x++) {
      for (let y = 0; y < 8; y++) {
        ...
        // see if we have something defined in the macros for this key
        console.log({ button });
        if (!macros[button]) {
          console.log(`No macro defined for button ${button}`);
          this.output.sendMessage([144, button, 0]);
          continue;
        }
